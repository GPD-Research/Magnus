...
export function SceneEquipmentGlyph({ definition }: SceneEquipmentGlyphProps) {
  ...
  switch (definition.glyph) {
    ...
    case 'downed-tree':
      return <><path d={`M ${-width * .12} ${halfLength} L ${-width * .2} ${-halfLength} L ${width * .2} ${-halfLength} L ${width * .12} ${halfLength} Z`} fill={definition.color} stroke="#2c2118" strokeWidth=".8" /><path d={`M 0 ${-length * .05} L ${-halfWidth} ${-length * .3} M 0 ${-length * .28} L ${halfWidth} ${-length * .48} M 0 ${length * .12} L ${width * .42} ${length * .3}`} fill="none" stroke="#31583a" strokeWidth="1.6" strokeLinecap="round" /><circle className="catalog-tree-canopy" cx={-halfWidth} cy={-length * .3} r={width * .18} fill="#416f45" /><circle className="catalog-tree-canopy" cx={halfWidth} cy={-length * .48} r={width * .2} fill="#416f45" /><circle className="catalog-tree-canopy" cx={width * .42} cy={length * .3} r={width * .17} fill="#416f45" /></>
    case 'jackknife':
